...
{
    load_plugin_textdomain('paydo-woocommerce', false, plugin_basename(dirname(__FILE__)) . '/languages');

    if (!class_exists('WC_Payment_Gateway')) {
        return;
    }
    if (class_exists('WC_Paydo')) {
        return;
    }
    ...
    class WC_Paydo extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            //Save options
            add_action('woocommerce_update_options_payment_gateways_paydo', [$this, 'process_admin_options']);
        ...
        <?php else : ?>
            <div class="inline error">
                <p>
                    <strong><?php _e('Gateway is disabled', 'paydo-woocommerce'); ?></strong>:
                    <?php _e('PayDo does not support the currency of your store.', 'paydo-woocommerce'); ?>
                </p>
            </div>
        <?php
        endif;
        ...
        public function init_form_fields()
        {
            ...
            $this->form_fields = array(
                ...
                'payment_form_language' => array(
                    'title' => __('Payment form language', 'paydo-woocommerce'),
                    'type' => 'select',
                    'description' => __('Select the language of the payment form for your store', 'paydo-woocommerce'),
                    'default' => 'en',
                    'options' => array(
                        'en' => __('English', 'paydo-woocommerce'),
                        'ru' => __('Russian', 'paydo-woocommerce'),
                    ),
                ),
                'jwt_token' => array(
                    'title' => __('JWT Token', 'paydo-woocommerce'),
                    'type' => 'text',
                    'description' => __('Required for Directpay! Issued in the client panel https://paydo.com/en/profile/settings/jwt-token', 'paydo-woocommerce'),
                    'default' => '',
                ),
...
The unavailability of the payment method or not all submitted fields can make payment impossible</span>', 'paydo-woocommerce') : "<span style='color: red'>JWT TOKEN REQUIRED FOR THIS FEATURE!!! </span><br>" . __('Select default payment method to directpay. (In list only available to your account payment methods.) <br><span style="color: red;">This function is in experimental mode! Use this option with caution.<br>
The unavailability of the payment method or not all submitted fields can make payment impossible</span>', 'paydo-woocommerce'),
...
//
        ...
        public function generate_form( $order_id )
        {
            ...
            $arrData['order']['description'] = __('Payment order #', 'paydo-woocommerce') . $order_id;
            $arrData['order']['items'] = [];
            ...
            $arrData['resultUrl'] = get_site_url() . "/?wc-api=wc_paydo&paydo=success&orderId={$order_id}";

            $arrData['failPath'] = get_site_url() . "/?wc-api=wc_paydo&paydo=fail&orderId={$order_id}";
            ...
            $action_adr = 'https://paydo.com/' . $this->language . '/payment/invoice-preprocessing/' . $response;
            ...
            return '<form action="' . esc_url($action_adr) . '" method="GET" id="paydo_payment_form">' . "\n" .
                implode("\n", $args_array) .
                '<input type="submit" class="button alt" id="submit_paydo_payment_form" value="' . __('Pay', 'paydo-woocommerce') . '" /> <a class="button cancel" href="' . $order->get_cancel_order_url() . '">' . __('Refuse payment & return to cart', 'paydo-woocommerce') . '</a>' . "\n" .
                '</form>';
        }
        ...
        public function check_ipn_response()
        {
            ...
            $requestType = !empty($_GET['paydo']) ? $_GET['paydo'] : '';
            ...
            switch ($requestType) {
                case 'result':
                    ...
                    if ($valid === 'V2'){
                        ...
                        do_action('paydo-ipn-request', $postedData);
                    } elseif ($valid = 'V1') {
    ...
    add_filter('woocommerce_payment_gateways', 'add_paydo_gateway');
    ...
    {
        $links[] = '<a href="' .
            admin_url('admin.php?page=wc-settings&tab=checkout&section=paydo') .
            '">' . __('Settings') . '</a>';
        return $links;
    }
